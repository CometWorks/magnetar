...
**Project:** Shared · **Namespace:** `Pulsar.Shared` · **Kind:** static class (+ enums) · **Lines:** 149

## Summary
Parses Magnetar's own command-line switches once at startup (in a static constructor) and exposes them as read-only boolean/enum flags for the rest of the loader. These are dash-prefixed arguments (e.g. `-noupdate`, `-debug`, `-sources`) layered on top of the SE DS's normal arguments, controlling update behavior, debug tooling, plugin compilation, mod-trust hardening, daemon detach, telemetry consent, and the `-help` screen. It also renders the human-readable usage text printed by `-help`.
...
- **Properties:** `UpdateType` — `None` if `-noupdate`, `Tester` if `-prerelease`, else `Standard`; `ExternalDebug` — `-debug`; `DebugMenu` — `-f12menu`; `CustomSources` — `-sources`; `ContinueGame` — `-continue`; `CheckAllPlugins` — `-debugCompileAll` (compile every listed plugin to surface build failures); `GameIntroVideo` — `-keepintro`; `MakeCheckFile` — `-mkcheck`; `TrustedMods` — `-hardened`; `Daemon` — `-daemon` (detach from the parent process at startup; see [Daemon.cs](../Legacy/Launcher/Daemon.cs.md)); `Consent` — a `ConsentChoice` resolved from `-withdraw-consent` / `-consent` / `-noconsent` (in that precedence), default `Unset`; `Help` — `true` for `-h`, `-help`, or `--help`. All are `{ get; private set; }`.
- **Methods:**
  - `LogFlags()` — builds the list of enabled non-default flags (including the `Consent` choice when not `Unset`) and writes a single `Enabled flags: ...` line via `LogFile` (nothing if none changed).
...
- **Used by:** [Interim.cs](../Legacy/Compiler/Interim.cs.md), [Game.cs](../Legacy/Launcher/Game.cs.md), [PluginLoader.cs](../Legacy/Loader/PluginLoader.cs.md), [Patch_MySessionLoader.cs](../Legacy/Patch/Patch_MySessionLoader.cs.md), [Program.cs](../Legacy/Program.cs.md), [PluginData.cs](Data/PluginData.cs.md), [Loader.cs](Loader.cs.md), [Tools.cs](Tools.cs.md), [Updater.cs](Updater.cs.md), [ConsentManager.cs](Votes/ConsentManager.cs.md)
