...
## Types
### `PluginVote` — class, public
A plain DTO deserialized from the JSON body of a `GET /Stats` response. Consumed by `PluginVotes.GetVotesForPlugin` and surfaced through `ConfigManager.Stats` to the UI layer.
...
- **Used by:** [StatsClient.cs](../StatsClient.cs.md), [PluginVotes.cs](PluginVotes.cs.md)
