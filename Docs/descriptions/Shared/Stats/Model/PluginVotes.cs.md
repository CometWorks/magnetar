...
Aggregates all per-plugin statistics into a keyed dictionary and provides a convenience accessor. Deserialized by `StatsClient.DownloadVotes` from the JSON response body and stored in `ConfigManager.Instance.Stats`.
