...
**Project:** Legacy · **Namespace:** `Pulsar.Legacy` · **Kind:** static class (program entry point) · **Lines:** 464
...
- **Constants:**
  ...
  - `VotesServer` — `"https://magnetarstats.ferenczi.eu"`, the stats endpoint passed to the shared loader.
- **Methods:**
  - `Main(args)` — entry point. Always captures launch state via `ServerControl.CaptureLaunchState`. On `NETCOREAPP`: computes `baseDir`/`Libraries/MagnetarInterim`/runtime dir; detects `-h`/`-help`/`--help` directly from the raw `args` (not via `Flags`, so no `Magnetar.Shared` type is pulled in before the resolver is installed) and, when help is requested, *skips* `NativeLibraryPreloader.Initialize` so the help screen does not load native libs or register lifecycle handlers that emit noise; otherwise on Linux calls `NativeLibraryPreloader.Initialize(baseDir)` before any `DllImport` fires. It then installs a generic `AssemblyResolve` over `[libraryDir, runtimeDir, baseDir]` (deliberately avoids referencing any `Magnetar.Shared` type so the CLR doesn't JIT-load Shared before the resolver is installed), then calls `MagnetarMain`. On Framework, `Main` and `MagnetarMain` are the same body.
  - `MagnetarMain(args)` (Core) / continuation of `Main` (Framework) — first, if `Flags.Help` is set, calls `Flags.PrintHelp()` and returns (no server start). Otherwise installs `OnUnhandledException`, the native crash handler, optional debugger launch (`Flags.ExternalDebug`), then `SetupCoreData`. Immediately after core data is ready it handles telemetry consent: if `Flags.Consent == ConsentChoice.Withdraw` it calls `ConsentManager.Withdraw(VotesServer)` (erase server-side data, record denial) and returns without starting the server; otherwise it calls `ConsentManager.Resolve()` to reconcile/prompt for consent. It then continues the pipeline: (`Daemon.Detach` when `Flags.Daemon`) → `TryUpdate` → `SetupGameData` → `CheckCanStart` → `SetupSteam` → `SetupPlugins` → `SetupGame`.
  ...
  - `SetupPlugins(baseDir)` — builds a `CompilerFactory` over `[ds64Dir, dependencyDir]` (on Framework, `Init`s it only when not native), wires `Tools.Init` with an `ExternalTools` + compiler, creates the shared `Loader` (`VotesServer`, core plugins). Then constructs a `Preloader` from the loaded plugins and, unless safe mode, runs `PreHooks` → `Patch(ds64Dir, preloadDir)` → `SetupGameResolver` → `PostHooks`; otherwise just `SetupGameResolver`.
...
## Cross-references
- **Uses:** `Pulsar.Legacy.Launcher` (`Daemon`, `Folder`, `Game`, `GameLog`, `ServerControl`); `Pulsar.Legacy.Compiler.CompilerFactory` (`Legacy/Compiler/`); `Pulsar.Legacy.Loader` (`PluginLoader`, `Preloader`, `NativeLibraryPreloader`); `Pulsar.Legacy.Patch.Patch_PrepareCrashReport` (`Legacy/Patch/`); `Pulsar.Shared` (`Tools`, `Flags`, `ConsentChoice`, `LogFile`, `Steam`, `Updater`, `Launcher`, `Loader`, `IExternalTools`); `Pulsar.Shared.Config` (`ConfigManager`, `CoreConfig`, `RemoteHubConfig`); `Pulsar.Shared.Votes` (`ConsentManager`); `HarmonyLib` (Early patch category); external systems — Steam (Steamworks resolver), GitHub (Updater repo), the Magnetar stats server.
