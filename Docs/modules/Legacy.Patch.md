...
Provides all Harmony patches that adapt the Space Engineers Dedicated Server binary to Magnetar's in-process, headless, externally-configured hosting model. The patches strip the configuration UI/service branch, reroute lifecycle and chat paths, conditionally inject the MagnetarMod companion when Steam Workshop is available, enforce trusted mods, and capture script diagnostics.
...
| `Patch_MyDefinitionManager` | static class | [`Legacy/Patch/Patch_MyDefinitionManager.cs`](../descriptions/Legacy/Patch/Patch_MyDefinitionManager.cs.md) | Harmony Prefix on MyDefinitionManager.LoadData; applies fail-soft MagnetarMod policy, then appends available ModPlugin items from the active profile to SE's definition-loading list. |
...
| `Patch_MySessionLoader` | class | [`Legacy/Patch/Patch_MySessionLoader.cs`](../descriptions/Legacy/Patch/Patch_MySessionLoader.cs.md) | Two Harmony Prefixes on MySessionLoader multiplayer-load methods; apply fail-soft MagnetarMod policy, then strip untrusted mods in hardened mode. Missing Steam install-state service warns and fails closed. |
| `Patch_MyWorkshop` | static class | [`Legacy/Patch/Patch_MyWorkshop.cs`](../descriptions/Legacy/Patch/Patch_MyWorkshop.cs.md) | Harmony patch on MyWorkshop.DownloadWorldModsBlocking; applies MagnetarMod policy before download and expands legacy archives afterward. Without Steam Workshop, the companion is removed and startup continues. |
